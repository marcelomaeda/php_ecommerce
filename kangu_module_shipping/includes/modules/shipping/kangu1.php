...
class kangu1
{ 
  var $code, $cart, $title, $description, $enabled, $kangu1; 

 
  function __construct() 
  { 

	if (defined('MODULE_SHIPPING_KANGU1_IMAGE') && MODULE_SHIPPING_KANGU1_IMAGE) {
	$image_payment = ' ' . tep_image(MODULE_SHIPPING_KANGU1_IMAGE, (( defined('MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW') && (MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW != "") ) ? MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW : ''), '', '', ' align="absmiddle"');
	}

	$this->code = 'kangu1';

	if ( defined('MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW') && (MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW != "") ) {
	$this->title = MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW . $image_payment;
	}elseif ( defined('MODULE_SHIPPING_KANGU1_TEXT_TITLE') && (MODULE_SHIPPING_KANGU1_TEXT_TITLE != "") ) {
	$this->title = MODULE_SHIPPING_KANGU1_TEXT_TITLE . $image_payment;
	}

    if ( defined('MODULE_SHIPPING_KANGU1_TEXT_DESCRIPTION') && (MODULE_SHIPPING_KANGU1_TEXT_DESCRIPTION != "") ) {
    $this->description = MODULE_SHIPPING_KANGU1_TEXT_DESCRIPTION;
    }
    $this->sort_order = defined('MODULE_SHIPPING_KANGU1_SORT_ORDER') ? MODULE_SHIPPING_KANGU1_SORT_ORDER : 0;
    $this->icon = '';
    $this->enabled = ((defined('MODULE_SHIPPING_KANGU1_STATUS') && MODULE_SHIPPING_KANGU1_STATUS == 'True') ? true : false);
    $this->kangu1 = 1;
...
// EOF correção
    ...
    if (!$shipping = $this->calcula_frete_kangu1($cep_origem, $cep_destino, $peso, ''))
      $shipping = 0;

    // if (!$shipping_prazo = $this->calcula_prazo_kangu1($cep_origem, $cep_destino, $peso, ''))
	//   $shipping_prazo = 0;
	...
	if (defined('ENABLE_MARKETPLACE') && ENABLE_MARKETPLACE == 'true' && defined('STATUS_MARKETPLACE') && STATUS_MARKETPLACE == 'true') {
		...
		foreach ($seller_member_id as $seller_id){
			...
			$shipping_seller_price = $this->calcula_frete_kangu1($cep_origem, $cep_destino, $peso, $seller_id);
//			print_r($shipping_seller_price);
//			echo "aaaa";
			// $shipping_seller_prazo = $this->calcula_prazo_kangu1($cep_origem, $cep_destino, $peso, $seller_id);
			$shipping_seller_kangu1 += $shipping_seller_price['price'];
...
//			}
		...
		//		if ($product_store_has_free_shipping == true){
		//			$shipping = shipping_seller_kangu1;
		//		}else{
					$shipping = $shipping['price'] + $shipping_seller_kangu1;
		//		}
	...
	if ($shipping_prazo == 0) {
	$this->quotes = array('id' => $this->code, 'module' =>$this->title, 'methods' => array(array('id' => $this->code, 'title' => MODULE_SHIPPING_KANGU1_MSG, 'cost' =>$shipping_cost += ($shipping + MODULE_SHIPPING_KANGU1_HANDLING))));
	}else{
		if ($shipping_prazo > 1) {
		$this->quotes = array('id' => $this->code, 'module' =>$this->title, 'methods' => array(array('id' => $this->code, 'title' => MODULE_SHIPPING_KANGU1_TEXT_WAY_NEW . $shipping_prazo . MODULE_SHIPPING_KANGU1_TEXT_WAY_NEW11, 'cost' =>$shipping_cost += ($shipping + MODULE_SHIPPING_KANGU1_HANDLING))));
		}else{
		$this->quotes = array('id' => $this->code, 'module' =>$this->title, 'methods' => array(array('id' => $this->code, 'title' => MODULE_SHIPPING_KANGU1_TEXT_WAY_NEW . $shipping_prazo . MODULE_SHIPPING_KANGU1_TEXT_WAY_NEW1, 'cost' =>$shipping_cost += ($shipping + MODULE_SHIPPING_KANGU1_HANDLING))));
		}
	}
    ...
    if ($shipping > 0)
      ...
      return $this->quotes['error'] = MODULE_SHIPPING_KANGU1_INVALID_ZONE;
  ...
  { 
    ...
    { 
      $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_KANGU1_STATUS'");
  ...
  function install() 
  {
    ...
    tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Entrega via kangu (cálculo online)', 'MODULE_SHIPPING_KANGU1_STATUS', 'True', 'Ativar Entrega via kangu?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
	...
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (".
				...
				"date_added".
				") values (".
				"'Texto a ser exibido para o cliente na tela de opções de entrega', 'MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW', 'kangu - Correios - Pac', ".
	...
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (".
				...
				") values (".
				"'Imagem da forma de entrega', 'MODULE_SHIPPING_KANGU1_IMAGE', 'images/shipping_pac.png', ".
				"'Imagem da forma de entrega.', '6', '1', ".
				"'tep_cfg_input_field_pickupimage(', now())");

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Taxa de manuseio/embalagem', 'MODULE_SHIPPING_KANGU1_HANDLING', '0', 'Taxa de manuseio e embalagem para esta forma de envio (opcional).', '6', '0', now())");

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Largura Caixa', 'MODULE_SHIPPING_KANGU1_PACK_WIDTH', '', 'Se escolheu caixa este campo é obrigatório', '6', '0', now())");

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Altura Caixa', 'MODULE_SHIPPING_KANGU1_PACK_HEIGHT', '', 'Se escolheu caixa este campo é obrigatório', '6', '0', now())");

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Comprimento Caixa', 'MODULE_SHIPPING_KANGU1_PACK_LENGHT', '', 'Se escolheu caixa este campo é obrigatório', '6', '0', now())");
	

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Mensagem na tela de frete', 'MODULE_SHIPPING_KANGU1_MSG', 'Prazo de Entrega: 1 a 3 dias úteis após a postagem.', 'Mensagem que será exibida na tela de seleção da forma de envio.', '6', '0', now())");

	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Secret Token', 'MODULE_SHIPPING_KANGU1_SECRETTOKEN', '', 'Secret Token.', '6', '0', now())");
		
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Ordem de exibição', 'MODULE_SHIPPING_KANGU1_SORT_ORDER', '255', 'Determina a ordem de exibição na tela de seleção da forma de envio.', '6', '0', now())"); 
 } 
  ...
  function calcula_frete_kangu1($cep_origem, $cep_destino, $peso, $seller_id)  {
	...
	$products = $order->products;
	   ...
	   }elseif ( (defined('MODULE_SHIPPING_KANGU1_PACK_LENGHT')) && (MODULE_SHIPPING_KANGU1_PACK_LENGHT != '') ) {
	   $comprimento_ok = MODULE_SHIPPING_KANGU1_PACK_LENGHT;
	   }else{
	   $comprimento_ok = '25';
	   }
	   ...
	   if ($altura != '') {
	   $altura_ok = $altura;
	   }elseif ( (defined('MODULE_SHIPPING_KANGU1_PACK_HEIGHT')) && (MODULE_SHIPPING_KANGU1_PACK_HEIGHT != '') ) {
	   $altura_ok = MODULE_SHIPPING_KANGU1_PACK_HEIGHT;
	   }else{
	   $altura_ok = '16';
	   }
	   if ($altura_ok <= '2') {
	   ...
	   if ($largura != '') {
	   $largura_ok = $largura;
	   }elseif ( (defined('MODULE_SHIPPING_KANGU1_PACK_WIDTH')) && (MODULE_SHIPPING_KANGU1_PACK_WIDTH != '') ) {
	   $largura_ok = MODULE_SHIPPING_KANGU1_PACK_WIDTH;
	   }else{
	   $largura_ok = '16';
	   }
	   if ($largura_ok <= '11') {
...
//echo 'peso: ' . $peso;
	...
	curl_setopt($ch, CURLOPT_HTTPHEADER, array(
	  ...
	  "token: " . MODULE_SHIPPING_KANGU1_SECRETTOKEN,
...
//	die;
	...
	$response_calc_kangu1 = json_decode($response);
	$result_calculate = objectToArray($response_calc_kangu1);
...
//	  die;
	  ...
	  if (is_array($result_calculate)){
		  if (strstr($result_calculate[0]['transp_nome'], 'Pac')){
...
//	  print_r($result_array);
  ...
  function keys() 
  { 
     $keys = array('MODULE_SHIPPING_KANGU1_STATUS', 'MODULE_SHIPPING_KANGU1_HANDLING', 'MODULE_SHIPPING_KANGU1_SECRETTOKEN', 'MODULE_SHIPPING_KANGU1_PACK_WIDTH', 'MODULE_SHIPPING_KANGU1_PACK_HEIGHT', 'MODULE_SHIPPING_KANGU1_PACK_LENGHT', 'MODULE_SHIPPING_KANGU1_MSG', 'MODULE_SHIPPING_KANGU1_SORT_ORDER', 'MODULE_SHIPPING_KANGU1_TEXT_TITLE_NEW', 'MODULE_SHIPPING_KANGU1_IMAGE'); 
