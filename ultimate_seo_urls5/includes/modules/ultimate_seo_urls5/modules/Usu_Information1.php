 ...
 * @Id $Id:: Usu_Information1.php 92 2009-10-07 11:46:19Z Rob           $:  Full Details   
 */

class Usu_Information1 extends aDataMap {

  const MARKER = '-ia-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'apages_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'eip_ajax.php'; // The filename define of the file where this code will be actioned
  ...
  public function __construct(){
    if ( defined( 'FILENAME_EIP_AJAX' ) && defined( 'TABLE_PAGES' ) ) {
...
//      $this->base_query = "SELECT information_title FROM " . TABLE_INFORMATION . " WHERE information_id=':info_id' AND language_id=':languages_id' LIMIT 1";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $placeholders = array( ':apages_id', ':languages_id' );
