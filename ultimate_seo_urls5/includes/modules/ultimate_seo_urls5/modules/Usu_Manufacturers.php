 ...
 * @Id $Id:: Usu_Manufacturers.php 92 2009-10-07 11:46:19Z Rob         $:  Full Details   
 */

class Usu_Manufacturers extends aDataMap {

  const MARKER = '-m-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'manufacturers_id'; // _GET key like e.g. cPath or lPath etc.
  ...
  private $manufacturers_name; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $placeholders = array( ':manufacturers_id', ':languages_id' );
    ...
    */
    $this->manufacturers_name = $this->linkText( $row['manufacturers_name'] );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['manufacturers_name']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
