 ...
 * @Id $Id:: Usu_Links2.php 92 2009-10-07 11:46:19Z Rob                $:  Full Details   
 */

class Usu_Links2 extends aDataMap {

  const MARKER = '-links-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'lPath'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'links.php'; // The filename define of the file where this code will be actioned
  ...
  private $link_categories_name; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  public function __construct(){
    if ( defined( 'FILENAME_LINKS' ) && defined( 'TABLE_LINK_CATEGORIES_DESCRIPTION' ) ){
      ...
      $this->base_query = "SELECT link_categories_name FROM " . TABLE_LINK_CATEGORIES_DESCRIPTION . " WHERE link_categories_id=':lPath' AND language_id=':languages_id' LIMIT 1";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $this->link_categories_name = $this->linkText( $row['link_categories_name'] );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['link_categories_name']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
