 ...
 * @Id $Id:: Usu_Newsdesk_Reviews_Info.php 92 2009-10-07 11:46:19Z Rob $:  Full Details   
 */

class Usu_Newsdesk_Reviews_Info extends aDataMap {

  const MARKER = '-nri-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'newsdesk_reviews_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'newsdesk_reviews_info.php'; // The filename define of the file where this code will be actioned
  ...
  public function __construct(){
    if ( defined( 'FILENAME_NEWSDESK_REVIEWS_INFO' ) && defined( 'TABLE_NEWSDESK_DESCRIPTION' ) ){
      ...
      $this->base_query = "SELECT newsdesk_article_name FROM " . TABLE_NEWSDESK_DESCRIPTION . " WHERE newsdesk_reviews_id=':newsdesk_reviews_id' AND language_id=':languages_id' LIMIT 1";
