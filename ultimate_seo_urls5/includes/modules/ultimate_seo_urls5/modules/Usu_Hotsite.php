 ...
 * @Id $Id:: Usu_Information.php 92 2009-10-07 11:46:19Z Rob           $:  Full Details   
 */

class Usu_Hotsite extends aDataMap {

  const MARKER = '-hi-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'hpages_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'hotsite.php'; // The filename define of the file where this code will be actioned
  ...
  private $information_title; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  public function __construct(){
    if ( defined( 'FILENAME_HOTSITE' ) && defined( 'TABLE_HOTSITE_PAGES' ) ) {
      // The items with a colon : are placeholders which must match the $placeholders array in method acquire()
//      $this->base_query = "SELECT information_title FROM " . TABLE_INFORMATION . " WHERE information_id=':info_id' AND language_id=':languages_id' LIMIT 1";
	  $this->base_query = "select 
                              p.pages_id, 
                              p.status,
                              s.pages_title, 
                              s.pages_html_text                               
                            from 
                              " . TABLE_HOTSITE_PAGES . " p LEFT JOIN " .TABLE_HOTSITE_PAGES_DESCRIPTION . " s on p.pages_id = s.pages_id
                            where 
                              s.language_id = ':languages_id'
                            and
                              p.pages_id = ':hpages_id'";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $placeholders = array( ':hpages_id', ':languages_id' );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['information_title']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
    

     switch( true ){
       case $page == self::FILENAME:
         $url = $this->linkCreate( self::FILENAME, normalize_special_characters($link_text), self::MARKER, $full_path );
         break;
       default:
