 ...
 * @Id $Id:: Usu_Sellers.php 92 2009-10-07 11:46:19Z Rob         $:  Full Details   
 */

class Usu_Sellers extends aDataMap {

  const MARKER = '-si-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'seller_id'; // _GET key like e.g. cPath or lPath etc.
  ...
  private $seller_name; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  public function __construct(){
    ...
    $this->base_query = "SELECT seller_name FROM " . TABLE_CUSTOMERS . " WHERE customers_id=':seller_id' LIMIT 1";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $placeholders = array( ':seller_id', ':languages_id' );
    ...
    */
    $this->seller_name = $this->linkText( $row['seller_name'] );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['seller_name']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
