 ...
 * @Id $Id:: Usu_PopUpImage.php 92 2009-10-07 11:46:19Z Rob            $:  Full Details   
 */

class Usu_PopUpImage extends aDataMap {

  const MARKER = '-pi-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'pID'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = FILENAME_POPUP_IMAGE; // The filename define of the file where this code will be actioned
  ...
  private $link_text; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    $this->link_text = usu::$registry->vars['products_id'][$this->dependency_value]['link_text'];
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['link_text']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
