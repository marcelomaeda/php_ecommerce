 ...
 * @Id $Id:: Usu_Article_Info.php 92 2009-10-07 11:46:19Z Rob          $:  Full Details   
 */

class Usu_Article_Info extends aDataMap {

  const MARKER = '-a-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'articles_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'article_info.php'; // The filename define of the file where this code will be actioned
  ...
  private $articles_name; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  public function __construct(){
    if ( defined('FILENAME_ARTICLE_INFO') && defined('TABLE_ARTICLES_DESCRIPTION') ){
      ...
      $this->base_query = "SELECT articles_name FROM " . TABLE_ARTICLES_DESCRIPTION . " WHERE articles_id=':articles_id' AND language_id=':languages_id' LIMIT 1";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $this->articles_name = $this->linkText( $row['articles_name'] );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['articles_name']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
