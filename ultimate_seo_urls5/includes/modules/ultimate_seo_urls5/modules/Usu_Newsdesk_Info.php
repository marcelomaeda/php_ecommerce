 ...
 * @Id $Id:: Usu_Newsdesk_Info.php 92 2009-10-07 11:46:19Z Rob         $:  Full Details   
 */

class Usu_Newsdesk_Info extends aDataMap {

  const MARKER = '-n-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'newsdesk_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'newsdesk_info.php'; // The filename define of the file where this code will be actioned
  ...
  private $newsdesk_article_name; // property populated in the acquire() method, there may be multiples of these, rename in line with the query results
  ...
  public function __construct(){
    if ( defined('FILENAME_NEWSDESK_INFO') && defined('TABLE_NEWSDESK_DESCRIPTION') ){
      // The items with a colon : are placeholders which must match the $placeholders array in method acquire()
      $this->base_query = "SELECT newsdesk_article_name FROM " . TABLE_NEWSDESK_DESCRIPTION . " WHERE newsdesk_id=':newsdesk_id' AND language_id=':languages_id' LIMIT 1";
  ...
  protected function acquire( $base_path, $full_path ) {
    ...
    */
    $placeholders = array( ':newsdesk_id', ':languages_id' );
    ...
    */
    $this->newsdesk_article_name = $this->linkText( $row['newsdesk_article_name'] );
   ...
   */
   public function buildLink( $page, $valuepair, &$url, &$added_qs, $parameters ) {
     ...
     */
     $link_text = $reg_item['newsdesk_article_name']; // the property added in the acquire function this would have been set in the acquire method like $this->xxxname
