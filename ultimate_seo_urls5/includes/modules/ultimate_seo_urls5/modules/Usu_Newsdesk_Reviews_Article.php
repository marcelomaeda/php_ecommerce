 ...
 * @Id $Id:: Usu_Newsdesk_Reviews_Article.php 92 2009-10-07 11:46:19Z #$:  Full Details   
 */

class Usu_Newsdesk_Reviews_Article extends aDataMap {

  const MARKER = '-nra-';  // Could for example be -c- or -l- etc.
  const DEPENDENCY = 'newsdesk_article_id'; // _GET key like e.g. cPath or lPath etc.
  const FILENAME = 'newsdesk_reviews_article.php'; // The filename define of the file where this code will be actioned
  ...
  public function __construct(){
    if ( defined( 'FILENAME_NEWSDESK_REVIEWS_ARTICLE' ) && defined( 'TABLE_NEWSDESK_DESCRIPTION' ) ) {
      ...
      $this->base_query = "SELECT newsdesk_article_name FROM " . TABLE_NEWSDESK_DESCRIPTION . " WHERE newsdesk_article_id=':newsdesk_article_id' AND language_id=':languages_id' LIMIT 1";
