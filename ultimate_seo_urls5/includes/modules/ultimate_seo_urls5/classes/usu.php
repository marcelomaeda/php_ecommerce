 ...
 * @lastmod $Date:: 2009-10-05 10:09:57 +0100 (Mon, 05 Oct 2009)       $:  Date of last commit
 * @version $Rev:: 89                                                  $:  Revision of last commit
 * @Id $Id:: usu.php 89 2009-10-05 09:09:57Z Rob                       $:  Full Details   
 */
...
class usu {
  ...
  private $constants = array('SEO_URLS_ENABLED', 'SEO_URLS_TYPE', 'SEO_URLS_CACHE_DAYS', 'SEO_URLS_FILTER_SHORT_WORDS',
                             'SEO_CHAR_CONVERT_SET', 'SEO_REMOVE_ALL_SPEC_CHARS', 'SEO_URLS_USE_W3C_VALID', 'SEO_URLS_OUPUT_PERFORMANCE',
                             'SEO_ADD_CPATH_TO_PRODUCT_URLS', 'SEO_ADD_CAT_PARENT', 'SEO_URLS_CACHE_SYSTEM');
  ...
  */
  private static function characterConversionSet(){
    ...
    if ( defined( 'SEO_CHAR_CONVERT_SET' ) && tep_not_null( SEO_CHAR_CONVERT_SET ) ){
      self::$character_conversion = array();
      $comma_separation = explode( ',', SEO_CHAR_CONVERT_SET );
  ...
  */
  public function href_link( $page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true ){ 
    ...
    if ( (isset($this->querystring)) && ( count($this->querystring) > 0 ) ){
  ...
  private function buildLink( $page, $parameters ){
   ...
   foreach ( $pair_array as $index => $pair_string ){
     ...
     if ( isset( self::$modules[$valuepair[0]] ) && is_object( self::$modules[$valuepair[0]] ) ){
       ...
       // Remove cPath from the product uri if SEO_ADD_CPATH_TO_PRODUCT_URLS is not set to true
       if ( defined( 'SEO_ADD_CPATH_TO_PRODUCT_URLS' ) && ( SEO_ADD_CPATH_TO_PRODUCT_URLS != 'true' ) && ( $page === FILENAME_PRODUCT_INFO ) ) {
